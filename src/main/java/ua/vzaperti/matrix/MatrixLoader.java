...
import ua.vzaperti.util.com.COMByteListener;
import ua.vzaperti.util.com.COMCommunication;
import ua.vzaperti.util.media.MyMedia;
import ua.vzaperti.util.media.MyMedia.MySound;
...
public class MatrixLoader implements COMByteListener, MessageListener<MatrixEvent> {
	...
	private MySound sound1 = MyMedia.getInstance().getSound("dozer");

	private COMCommunication comPort;
	...
	public MatrixLoader() {
		...
		try {
			comPort = new COMCommunication(this);		
		} catch (Exception e) {
			log.error("", e);
		}
	}
	...
	class JuJitsuTask extends TimerTask{
		public void run(){			
			if(!juJitsuCompleteImg.isVisible())
			{
				...
				juJitsuSlides.setIcon(getImage("jujitsu/"+Images.FIRST_SKILL_SLIDES[i]));			//upload disk
				i++;
				if(i == 28){
					disk1 = true;
					juJitsuUploadImg.setVisible(false);
				...
				juJitsuSlides.setIcon(getImage("jujitsu/"+Images.FIRST_SKILL_SLIDES[27]));		//insert into complete program				
			}
		}
	}
	...
	private void juJitsuStop(){
		...
		juJitsuSlides.setIcon(getImage("jujitsu/"+Images.FIRST_SKILL_SLIDES[0]));	//output from complete program
		i = 0;
		if(!juJitsuCompleteImg.isVisible())
		{
	...
	class DefenseTask extends TimerTask{
		public void run(){			
			if(!defenseCompleteImg.isVisible())
			{
				...
				defenseSlides.setIcon(getImage("defence/"+Images.SECOND_SKILL_SLIDES[i]));			//upload disk
				i++;
				if(i == 23){
					disk2 = true;
					defenseUploadImg.setVisible(false);
				...
				progressBar.setValue(100);
				defenseSlides.setIcon(getImage("defence/"+Images.SECOND_SKILL_SLIDES[22]));		//insert into complete program							
			}
		}
	}
	...
	private void defenseStop(){
		...
		defenseSlides.setIcon(getImage("defence/"+Images.SECOND_SKILL_SLIDES[0]));		//output from complete program
		i = 0;
		if(!defenseCompleteImg.isVisible())
		{
	...
	class SpSkillsTask extends TimerTask{
		public void run(){
			if(!spSkillsCompleteImg.isVisible() && (i != 19 || cureDisk3))
			{
				...
				spSkillsSlides.setIcon(getImage("sp_skills/"+Images.THIRD_SKILL_SLIDES[i]));			//upload disk
				i++;				
				if(i == 25){
					disk3 = true;
					spSkillsUploadImg.setVisible(false);
...
//					}
				...
				if(cureDisk3){					
					progressBar.setValue(100);
					spSkillsSlides.setIcon(getImage("sp_skills/"+Images.THIRD_SKILL_SLIDES[24]));		//insert into complete program					
				}
			}
		}
	}
	...
	private void spSkillsStop(){
		...
		spSkillsSlides.setIcon(getImage("sp_skills/"+Images.THIRD_SKILL_SLIDES[0]));		//output from complete program
		i = 0;
	...
	class AttackTask extends TimerTask{
		public void run(){
			try {
				...
				spSkillsSlides.setIcon(getImage("sp_skills/"+Images.THIRD_SKILL_SLIDES[18]));
				if(attackImg.isVisible()){
					attackImg.setVisible(false);
					comPort.write((byte)MatrixLoaderEvents.RED_LAMP_OFF.ordinal());
					log.info("Lamp OFF");
				} else {
					attackImg.setVisible(true);
					comPort.write((byte)MatrixLoaderEvents.RED_LAMP_ON.ordinal());
					log.info("Lamp ON");
				}
			} catch (Exception e) {
				log.error("", e);
